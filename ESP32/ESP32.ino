...
const char *username        = "YOUR_USERNAME_DUINOCOIN";
const char *rig_identifier  = "ESP32";
...
void Task1code(void *pvParameters) {
  ...
  for (;;) {
    ...
    while (client_one.connected()) {
      esp_task_wdt_reset();
      Serial.println("CORE1 New job for user: " +
                     String(username));
      client_one.flush();
...
void Task2code(void *pvParameters) {
  ...
  for (;;) {
    ...
    while (client.connected()) {
      esp_task_wdt_reset();
      Serial.println("CORE2 New job for user: " +
                     String(username));
